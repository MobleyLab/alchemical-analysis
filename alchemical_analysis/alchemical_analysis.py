...
parser = OptionParser()
parser.add_option('-a', '--software', dest='software',
                  help='Package\'s name the data files come from: Gromacs, Sire, Desmond, or AMBER. Default: Gromacs.', default='Gromacs')
parser.add_option('-c', '--cfm', dest='bCFM',
                  help='The Curve-Fitting-Method-based consistency inspector. Default: False.', default=False, action='store_true')
parser.add_option('-d', '--dir', dest='datafile_directory',
                  help='Directory in which data files are stored. Default: Current directory.', default='.')
parser.add_option('-f', '--forwrev', dest='bForwrev',
                  help='Plot the free energy change as a function of time in both directions, with the specified number of points in the time plot. The number of time points (an integer) must be provided. Default: 0', default=0, type=int)
parser.add_option('-g', '--breakdown', dest='breakdown',
                  help='Plot the free energy differences evaluated for each pair of adjacent states for all methods, including the dH/dlambda curve for TI. Default: False.', default=False, action='store_true')
parser.add_option('-i', '--threshold', dest='uncorr_threshold',
                  help='Proceed with correlated samples if the number of uncorrelated samples is found to be less than this number. If 0 is given, the time series analysis will not be performed at all. Default: 50.', default=50, type=int)
parser.add_option('-k', '--koff', dest='bSkipLambdaIndex',
                  help='Give a string of lambda indices separated by \'-\' and they will be removed from the analysis. (Another approach is to have only the files of interest present in the directory). Default: None.', default='')
...
parser.add_option('-n', '--uncorr', dest='uncorr',
...
parser.add_option('-o', '--out', dest='output_directory',
                  help='Directory in which the output files produced by this script will be stored. Default: Same as datafile_directory.', default='')
parser.add_option('-p', '--prefix', dest='prefix',
                  help='Prefix for datafile sets, i.e.\'dhdl\' (default).', default='dhdl')
parser.add_option('-q', '--suffix', dest='suffix',
                  help='Suffix for datafile sets, i.e. \'xvg\' (default).', default='xvg')
parser.add_option('-r', '--decimal', dest='decimal',
                  help='The number of decimal places the free energies are to be reported with. No worries, this is for the text output only; the full-precision data will be stored in \'results.pickle\'. Default: 3.', default=3, type=int)
parser.add_option('-s', '--skiptime', dest='equiltime',
                  help='Discard data prior to this specified time as \'equilibration\' data. Units picoseconds. Default: 0 ps.', default=0, type=float)
parser.add_option('-t', '--temperature', dest='temperature',
                  help="Temperature in K. Default: 298 K.", default=298, type=float)
parser.add_option('-u', '--units', dest='units',
                  help='Units to report energies: \'kJ\', \'kcal\', and \'kBT\'. Default: \'kJ\'', default='kJ')
parser.add_option('-v', '--verbose', dest='verbose',
                  help='Verbose option. Default: False.', default=False, action='store_true')
parser.add_option('-w', '--overlap', dest='overlap',
                  help='Print out and plot the overlap matrix. Default: False.', default=False, action='store_true')
parser.add_option('-x', '--ignoreWL', dest='bIgnoreWL',
                  help='Do not check whether the WL weights are equilibrated. No log file needed as an accompanying input.', default=False, action='store_true')
parser.add_option('-y', '--tolerance', dest='relative_tolerance',
                  help="Convergence criterion for the energy estimates with BAR and MBAR. Default: 1e-10.", default=1e-10, type=float)
parser.add_option('-z', '--initialize', dest='init_with',
                  help='The initial MBAR free energy guess; either \'BAR\' or \'zeros\'. Default: \'BAR\'.', default='BAR')
...
def getMethods(string):
    ...
    all_methods = ['TI', 'TI-CUBIC', 'DEXP', 'IEXP',
                   'GINS', 'GDEL', 'BAR', 'UBAR', 'RBAR', 'MBAR']
...
def uncorrelate(sta, fin, do_dhdl=False):
    ...
    # u_kln[k,m,n] is the reduced potential energy of uncorrelated sample
    # index n from state k evaluated at state m
    ...
    UNCORR_OBSERVABLE = {'Gromacs': P.uncorr, 'Amber': 'dhdl',
                         'Sire': 'dhdl', 'Desmond': 'dE'}[P.software.title()]
    ...
    if UNCORR_OBSERVABLE == 'dhdl':
        ...
        for k in range(K):
            ...
            # Determine indices of uncorrelated samples from potential
            # autocorrelation analysis at state k

            # NML: Set statistical inefficiency (g) = 1 if vector is all 0
            if not numpy.any(dhdl_sum):
                # print "WARNING: Found all zeros for Lambda={}\n Setting
                # statistical inefficiency g=1.".format(k)
                g[k] = 1
            else:
            ...
            indices = sta[
                k] + numpy.array(pymbar.timeseries.subsampleCorrelatedData(dhdl_sum, g=g[k]))
            N = len(indices)  # number of uncorrelated samples
            # Handle case where we end up with too few.
            if N < P.uncorr_threshold:
                if do_dhdl:
                    print(
                        "WARNING: Only %s uncorrelated samples found at lambda number %s; proceeding with analysis using correlated samples..." % (N, k))
                indices = sta[k] + numpy.arange(len(dhdl_sum))
                N = len(indices)
            # Store the number of uncorrelated samples from state k.
            N_k[k] = N
    ...
    if UNCORR_OBSERVABLE == 'dhdl_all':
        ...
        for k in range(K):
            # Sum up over the energy components; notice, that only the relevant
            # data is being used in the third dimension.
    ...
    if UNCORR_OBSERVABLE == 'dE':
        ...
        for k in range(K):
            ...
            dE = u_klt[k, k + 1, sta[k]:fin[k]] if not k == K - \
                1 else u_klt[k, k - 1, sta[k]:fin[k]]

            g[k] = pymbar.timeseries.statisticalInefficiency(dE)
            # indices of uncorrelated samples
            indices = sta[
                k] + numpy.array(pymbar.timeseries.subsampleCorrelatedData(dE, g=g[k]))
            N = len(indices)  # number of uncorrelated samples
            # Handle case where we end up with too few.
            if N < P.uncorr_threshold:
...
def estimatewithMBAR(u_kln, N_k, reltol, regular_estimate=False):
    ...
    def plotOverlapMatrix(O):
        ...
        for i in range(K):
            ...
            for j in range(K):
                if O[j, i] < 0.005:
                    ...
                    ii = ("%.2f" % O[j, i])[1:]
                alf = O[j, i] / max_prob
                pl.fill_between([i, i + 1], [K - j, K - j],
                                [K - (j + 1), K - (j + 1)], color='k', alpha=alf)
                pl.annotate(ii, xy=(i, j), xytext=(i + 0.5, K - (j + 0.5)), size=8,
                            textcoords='data', va='center', ha='center', color=('k' if alf < 0.5 else 'w'))
        ...
        if P.bSkipLambdaIndex:
            ...
            ks = list(range(K))
        for i in range(K):
            pl.annotate(ks[i], xy=(i + 0.5, 1), xytext=(i + 0.5, K + 0.5), size=10,
                        textcoords=('data', 'data'), va='center', ha='center', color='k')
            pl.annotate(ks[i], xy=(-0.5, K - (j + 0.5)), xytext=(-0.5, K - (i + 0.5)),
                        size=10, textcoords=('data', 'data'), va='center', ha='center', color='k')
        pl.annotate('$\lambda$', xy=(-0.5, K - (j + 0.5)), xytext=(-0.5, K + 0.5),
                    size=10, textcoords=('data', 'data'), va='center', ha='center', color='k')
        ...
        pl.xlim(-1, K)
        pl.ylim(0, K + 1)
        pl.savefig(os.path.join(P.output_directory, 'O_MBAR.pdf'),
    ...
    MBAR = pymbar.mbar.MBAR(u_kln, N_k, verbose=P.verbose,
                            relative_tolerance=reltol, initialize=P.init_with)
    # Get matrix of dimensionless free energy differences and uncertainty
    # estimate.
...
class naturalcubicspline:
    ...
    def __init__(self, x):
        ...
        # define the H and M matrix, from p. 371 "applied numerical methods
        # with matlab, Chapra"
        H[0, 0] = 1
        H[L - 1, L - 1] = 1
        ...
        CW = numpy.dot(numpy.linalg.inv(H), M)
        ...
        for k in range(0, L - 1):
            w = DW[k, :] * (h[k]**4) / 4.0 + CW[k, :] * (h[k]**3) / \
                3.0 + BW[k, :] * (h[k]**2) / 2.0 + AW[k, :] * (h[k])
            self.wk[k, :] = w
            self.wsum += w
    ...
    def interpolate(self, y, xnew):
        ...
        # get the array of actual coefficients by multiplying the coefficient
...
def get_lchange(lv):
    ...
    lchange = numpy.zeros([K, n_components], bool)
    for j in range(n_components):
        # need to identify range over which lambda doesn't change, and not
        # interpolate over that range.
        for k in range(K - 1):
...
def TIprelim(lv):
    ...
    # Compute <dhdl> and std(dhdl) for each component, for each lambda;
    # multiply them by beta to make unitless.
    ...
    for k in range(K):
        ...
        std_dhdl[k, :] = P.beta * \
            numpy.std(dhdl[k, :, 0:N_k[k]], axis=1) / numpy.sqrt(N_k[k] - 1)
    ...
    for k in range(K - 1):
        ...
        for name in P.methods:
            ...
            if name == 'TI':
                ...
                #==============================================================
                df['TI'] = 0.5 * \
                    numpy.dot(dlam[k], (ave_dhdl[k] + ave_dhdl[k + 1]))
                ddf['TI'] = 0.5 * \
                    numpy.sqrt(
            ...
            if name == 'BAR':
                ...
                #==============================================================
                (df['BAR'], ddf['BAR']) = pymbar.bar.BAR(w_F, w_R,
                                                         relative_tolerance=P.relative_tolerance, verbose=P.verbose)
            ...
            if name == 'RBAR':
                ...
                for trial_udf in range(-10, 10, 1):
                    (udf, uddf) = pymbar.bar.BAR(w_F, w_R, DeltaF=trial_udf,
                                                 iterated_solution=False, verbose=P.verbose)
            ...
            if name == 'MBAR':
                ...
                #==============================================================
                (df['MBAR'], ddf['MBAR']) = Deltaf_ij[
                    k, k + 1], numpy.nan_to_num(dDeltaf_ij[k, k + 1])
...
def totalEnergies():
    ...
    if endcoul > endvdw:
        # If it comes after the vdW section, for now assume it follows vdW
        # (will need correcting for case where they are mixed together)
        startcoul = endvdw
        startvdw = 0
    ...
    # Perform the energy segmentation; be pedantic about the TI cumulative
    # ddF's (see Section 3.1 of the paper).
            ...
            if name[0:2] == 'TI':
                ...
                for j in jlist:
                    ...
                    if not (lj == False).all():  # handle the all-zero lv column
                        if name == 'TI-CUBIC':
                            ddF[name] += numpy.dot((cubspl[j].wsum)
                                                   ** 2, std_dhdl[lj, j]**2)
                        elif name == 'TI':
                            h = numpy.trim_zeros(dlam[:, j])
                            wsum = 0.5 * (numpy.append(h, 0) +
                                          numpy.append(0, h))
    ...
    def printLine(str1, str2, d1=None, d2=None):
        ...
        for name in P.methods:
            ...
            if d1 and d2:
                print(str2 % (d1[name] / P.beta_report,
                              d2[name] / P.beta_report), end=' ')
                text += ' ' + \
                    str2 % (d1[name] / P.beta_report, d2[name] / P.beta_report)
        print('')
        outtext.append(text + '\n')
        return
    ...
    outfile = open(os.path.join(P.output_directory, 'results.pickle'), 'wb')
...
def dF_t():
    ...
    def plotdFvsTime(f_ts, r_ts, F_df, R_df, F_ddf, R_ddf):
        ...
        line0 = pl.fill_between([r_ts[0], f_ts[-1]], R_df[0] -
                                R_ddf[0], R_df[0] + R_ddf[0], color='#D2B9D3', zorder=-5)
        for i in range(len(f_ts)):
            line1 = pl.plot([f_ts[i]] * 2, [F_df[i] - F_ddf[i], F_df[i] + F_ddf[i]],
                            color='#736AFF', ls='-', lw=3, solid_capstyle='round', zorder=1)
        line11 = pl.plot(f_ts, F_df, color='#736AFF', ls='-', lw=3,
                         marker='o', mfc='w', mew=2.5, mec='#736AFF', ms=12, zorder=2)

        for i in range(len(rr_ts)):
            line2 = pl.plot([rr_ts[i]] * 2, [R_df[i] - R_ddf[i], R_df[i] + R_ddf[i]],
                            color='#C11B17', ls='-', lw=3, solid_capstyle='round', zorder=3)
        line22 = pl.plot(rr_ts, R_df, color='#C11B17', ls='-', lw=3,
                         marker='o', mfc='w', mew=2.5, mec='#C11B17', ms=12, zorder=4)
        ...
        leg = pl.legend((line1[0], line2[0]), (r'$Forward$',
                                               r'$Reverse$'), loc=9, prop=FP(size=18), frameon=False)
        pl.xlabel(
            r'$\mathrm{Fraction\/of\/the\/simulation\/time}$', fontsize=16, color='#151B54')
        pl.ylabel(r'$\mathrm{\Delta G\/%s}$' %
                  P.units, fontsize=16, color='#151B54')
    ...
    if not (P.snap_size[0] == numpy.array(P.snap_size)).all():
        parser.error(
            "\nThe snapshot size isn't the same for all the files; cannot perform the dF(t) analysis.")

    # Define a list of bForwrev equidistant time frames at which the free
    # energy is to be estimated; count up the snapshots embounded between the
    # time frames.
    n_tf = P.bForwrev + 1
    ...
    if P.bExpanded:
        ...
        tf[0] = 0
        for i in range(n_tf - 1):
            nss_tf[i + 1] = numpy.array([min(j, tf[i + 1])
                                         for j in nsnapshots]) - numpy.sum(nss_tf[:i + 1], axis=0)

    # Define the real time scale (in ps) rather than a snapshot sequence.
    ts = ["%.1f" % ((i - (i != tf[0])) * P.snap_size[0] + P.equiltime)
          for i in tf]
    ...
    # Store the MBAR energy that accounts for all the equilibrated snapshots
    # (has already been computed in the previous section).
    F_df[-1], F_ddf[-1] = (Deltaf_ij[0, K - 1] / P.beta_report,
                           dDeltaf_ij[0, K - 1] / P.beta_report)
    R_df[0], R_ddf[0] = (Deltaf_ij[0, K - 1] / P.beta_report,
                         dDeltaf_ij[0, K - 1] / P.beta_report)
    ...
    for i in range(n_tf - 2):
        ...
        R_df[i + 1], R_ddf[i +
                           1] = estimatewithMBAR(u_kln, N_k, P.relative_tolerance)
   ...
   reverse and forward (in parentheses) direction.\n""" % P.units)
    ...
    print("%10s -- %s\n%10s -- %-10s %16s %15.3f +- %5.3f\n%s" %
          (ts[-1], ts[-1], '(' + ts[0], ts[-1] + ')', 'XXXXXX', F_df[-1], F_ddf[-1], 70 * '-'))

    # Plot the forward and reverse dF(t); store the data points in the text
    # file.
    print("Plotting data to the file dF_t.pdf...\n\n")
    plotdFvsTime([float(i) for i in ts[1:]], [float(i)
                                              for i in ts[:-1]], F_df, R_df, F_ddf, R_ddf)
...
def plotdFvsLambda():
    ...
    def plotdFvsLambda1():
        ...
        for name in P.methods:
            ...
            line = pl.bar(x + len(lines) * width, y, width, color=colors[
                          name], yerr=ye, lw=0.1 * elw, error_kw=dict(elinewidth=elw, ecolor='black', capsize=0.5 * elw))
            lines += (line[0],)
        pl.xlabel('States', fontsize=12, color='#151B54')
        pl.ylabel('$\Delta G$ ' + P.units, fontsize=12, color='#151B54')
        pl.xticks(x + 0.5 * width * len(P.methods),
        ...
        leg = pl.legend(lines, tuple(P.methods), loc=3, ncol=2,
                        prop=FP(size=10), fancybox=True)
        leg.get_frame().set_alpha(0.5)
        pl.title('The free energy change breakdown', fontsize=12)
        pl.savefig(os.path.join(P.output_directory,
                                'dF_state_long.pdf'), bbox_inches='tight')
        pl.close(fig)
        return
    ...
    def plotdFvsLambda2(nb=10):
        ...
        for x in xs:
            ...
            for name in P.methods:
                ...
                ye = [ddf_allk[i][name] / P.beta_report for i in x]
                line = pl.bar(x + len(lines) * width, y, width, color=colors[
                              name], yerr=ye, lw=0.05 * elw, error_kw=dict(elinewidth=elw, ecolor='black', capsize=0.5 * elw))
                lines += (line[0],)
            ...
            for i in x + 0.5 * width * len(P.methods):
                ax.annotate('$\mathrm{%d-%d}$' % (i, i + 1), xy=(i, 0), xycoords=('data', 'axes fraction'),
                            xytext=(0, -2), size=10, textcoords='offset points', va='top', ha='center')
            pl.xlim(x[0], x[-1] + len(lines) * width + (mnb - len(x)))
            ndx += 1
        leg = ax.legend(lines, tuple(P.methods), loc=0, ncol=2, prop=FP(
            size=8), title='$\mathrm{\Delta G\/%s\/}\mathit{vs.}\/\mathrm{lambda\/pair}$' % P.units, fancybox=True)
        leg.get_frame().set_alpha(0.5)
        pl.savefig(os.path.join(P.output_directory,
                                'dF_state.pdf'), bbox_inches='tight')
        pl.close(fig)
        return
    ...
    def plotTI():
        ...
        for j in range(n_components):
            ...
            if not (y == 0).all():
                ...
                if 'TI' in P.methods:
                    ...
                    pl.plot(xlegend, [0 * wnum for wnum in xlegend], ls='-',
                            color=colors[ndx], label=lv_names2[ndx])  # for the paper
                    ...
                    if 'TI-CUBIC' in P.methods and not cubspl[j] == 0:
                        ...
                        pl.plot(xnew + ndx, ynew, color='#B6B6B4',
                                ls='-', solid_capstyle='round', lw=3.0)
                    ...
                    for i in range(len(x) - 1):
                        ...
                        max_y = max(ynew.max(), max_y)
                        if i % 2 == 0:
                            pl.fill_between(xnew + ndx, 0, ynew,
        ...
        for i, j in zip(xs[1:], xt[1:]):
            pl.annotate(('%.2f' % (i - 1.0 if i > 1.0 else i) if not j == '' else ''), xy=(i, 0), xytext=(i, 0.01),
                        size=10, rotation=90, textcoords=('data', 'axes fraction'), va='bottom', ha='center', color='#151B54')
        if ndx > 1:
        ...
        pl.ylabel(r'$\mathrm{\langle{\frac{ \partial U } { \partial \lambda }}\rangle_{\lambda}\/%s}$' %
                  P.units, fontsize=20, color='#151B54')
        pl.annotate('$\mathit{\lambda}$', xy=(0, 0), xytext=(
            0.5, -0.05), size=18, textcoords='axes fraction', va='top', ha='center', color='#151B54')
...
def plotCFM(u_kln, N_k, num_bins=100):
    ...
    def plotdg_vs_dU(yy, df_allk, ddf_allk):
        ...
        for i, (xx_i, yy_i) in enumerate(yy):
            ...
            ax.fill_between(xx_i, df_allk[i]['BAR'] - ddf_allk[i]['BAR'], df_allk[i][
                            'BAR'] + ddf_allk[i]['BAR'], color='#D2B9D3', zorder=-1)

            ax.annotate(r'$\mathrm{%d-%d}$' % (ks[i], ks[i + 1]), xy=(0.5, 0.9), xycoords=('axes fraction', 'axes fraction'), xytext=(
                0, -2), size=14, textcoords='offset points', va='top', ha='center', color='#151B54', bbox=dict(fc='w', ec='none', boxstyle='round', alpha=0.5))
            pl.xlim(xx_i.min(), xx_i.max())
        pl.annotate(r'$\mathrm{\Delta U_{i,i+1}\/(reduced\/units)}$', xy=(0.5, 0.03), xytext=(0.5, 0), xycoords=(
            'figure fraction', 'figure fraction'), size=20 + sf, textcoords='offset points', va='center', ha='center', color='#151B54')
        pl.annotate(r'$\mathrm{\Delta g_{i+1,i}\/(reduced\/units)}$', xy=(0.06, 0.5), xytext=(0, 0.5), rotation=90, xycoords=(
            'figure fraction', 'figure fraction'), size=20 + sf, textcoords='offset points', va='center', ha='center', color='#151B54')
    ...
    for k in range(0, K - 1):
        ...
        (counts_l, xbins_l) = numpy.histogram(
            left, bins=num_bins, range=(mi, ma))
        (counts_r, xbins_r) = numpy.histogram(
            righ, bins=num_bins, range=(mi, ma))
...
def main():
    ...
    # Simulation profile P (to be stored in 'results.pickle') will amass
    ...
    if P.software.title() == 'Gromacs':
        ...
        P.prefix = 'gibbs'
        P.suffix = 'dE'
        # Given only dE of adj Lambdas, limiting to BAR only
        P.methods = ['BAR']
        ...
        # u_klt[k,m,t] is the reduced potential energy of snapshot t of state k
        # evaluated at state m
    ...
    if (numpy.array(['Sire', 'Gromacs', 'Amber']) == P.software.title()).any():
        dhdl, N_k, u_kln = uncorrelate(sta=numpy.zeros(
            K, int), fin=nsnapshots, do_dhdl=True)
    elif P.software.title() == 'Desmond':
        N_k, u_kln = uncorrelate(sta=numpy.zeros(
            K, int), fin=nsnapshots, do_dhdl=False)
