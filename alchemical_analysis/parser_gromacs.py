...
def readDataGromacs(P):
    ...
    class F:
        ...
        def sortedHelper(self):
            ...
            meat = os.path.basename(self.filename).replace(
                P.prefix, '').replace(P.suffix, '')
            l = [i for i in re.split('\.|-|_', meat) if i]
            try:
                # Will be of use for selective MBAR analysis.
                self.state = l[0] = int(l[0])
            except:
                print(
        ...
        def readHeader(self):
            # Number of lines from the top that are to be skipped.
            self.skip_lines = 0
        ...
        def iter_loadtxt(self, state):
            ...
            def slice_data(data, state=state):
                ...
                data = data.T
                dhdlt[state, :, nsnapshots_l[state]:nsnapshots_r[
                    state]] = data[read_dhdl_sta: read_dhdl_end, :]

                if not bSelective_MBAR:
                    r1, r2 = (read_dhdl_end, read_dhdl_end +
                              (ndE[state] if not self.bExpanded else K))
                    if bPV:
                        u_klt[state, s1:s2, nsnapshots_l[state]:nsnapshots_r[
                            state]] = P.beta * (data[r1:r2, :] + data[-1, :])
                    else:
                        u_klt[state, s1:s2, nsnapshots_l[state]                              :nsnapshots_r[state]] = P.beta * data[r1:r2, :]
                # can't do slicing; prepare a mask (slicing is thought to be
                # faster/less memory consuming than masking)
                else:
                    mask_read_uklt = numpy.array([0] * read_dhdl_end + [1 if (
                        k in sel_states) else 0 for k in range(ndE[0])] + ([0] if bPV else []), bool)
                    if bPV:
                        u_klt[state, s1:s2, nsnapshots_l[state]:nsnapshots_r[
                            state]] = P.beta * (data[mask_read_uklt, :] + data[-1, :])
                    else:
                        u_klt[state, s1:s2, nsnapshots_l[state]:nsnapshots_r[
                            state]] = P.beta * data[mask_read_uklt, :]
                return
        ...
        def parseLog(self):
            ...
            if not(P.bIgnoreWL):
                ...
                try:
                    with open(logfilename, 'rb') as infile:
                        dt = float(unixlike.grepPy(
                            infile, s='delta-t').split()[-1])
                        WLstep = int(unixlike.grepPy(
                            infile, s='equilibrated').split()[1].replace(':', ''))
                except:
    ...
    if P.bSkipLambdaIndex:
        try:
            lambdas_to_skip = [int(l) for l in unixlike.trPy(
                P.bSkipLambdaIndex, '-').split()]
        except:
    ...
    for nf, f in enumerate(fs):
        ...
        if nf > 0:
            ...
            if temperature:
                ...
                print("Temperature not present in xvg files. Using %g K." %
                      P.temperature)
    ...
    if len(ndE_unique) == 1:  # [K]
        if not numpy.array([i == lv[0] for i in lv]).all():
            ...
            lv = lv[0]
            # Handle the case when only some particular files/lambdas are
            # given.
            if 1 < n_files < len(lv):
        ...
        bSelective_MBAR = False
        # Scenario #2: Have the adjacent states only; 2 dE columns for the
        # terminal states, 3 for inner ones.
        if ndE_unique.tolist() == [2, 3]:
            lv = [l[i > 0] for i, l in enumerate(lv)]
        # Scenario #3: Have a mixture of formats (adjacent and all): either
        # [2,3,K], or [2,K], or [3,K].
        else:
    ...
    nsnapshots = numpy.zeros((n_files, K), dtype=int)
    ...
    for nf, f in enumerate(fs):
        ...
        if f.bExpanded:
            ...
            extract_states = numpy.genfromtxt(
                f.filename, dtype=int, skiprows=f.skip_lines, skip_footer=1 * bLenConsistency, usecols=1)
            nsnapshots[nf] += numpy.array(list(Counter(extract_states).values()))
    ...
    maxn = max(nsnapshots.sum(axis=0))
    # dhdlt[k,n,t] is the derivative of energy component n with respect to
    # state k of snapshot t
    dhdlt = numpy.zeros([K, n_components, int(maxn)], float)
