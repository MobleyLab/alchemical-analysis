...
def trPy(s, l='[,\\\\"/()-]', char=' '):
    ...
    return re.sub(str(l), str(char), str(s))
...
def grepFromSection(f, section, *stringhe, **kwargs):
    ...
    n = 0 if not b'n' in kwargs else kwargs['n']
    ...
    for i, line in enumerate(f):
        if section in line:
            ...
            found = {}
            try:
                # The first line after the line with 'section'.
                line = f.next()
                i += 1
                ...
                while line.strip(b'\n '):
                    ...
                    line = f.next()
                    # Will account for the initial 0 when leaving the loop.
                    i += 1
